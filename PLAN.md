...
| Dashboard | Plain HTML/CSS/ES-module JS, no build step | Visualizes commit graph, renders diffs, drives replay comparisons. Chose the "move faster" option this row already named: a build toolchain (React) adds a second language ecosystem to a repo whose only runtime dependency was `typer`, and the one interesting piece — lane assignment against a real commit DAG — is ~110 lines a chart library wouldn't have supplied anyway. Reversible in the direction that matters, since the REST layer is plain JSON |
...
| Dashboard stack | Hand-written HTML/CSS/ES-module JS served by `StaticFiles` from inside the package; the commit graph is inline SVG generated from `/api/log`'s JSON in a single forward-pass layout. No React, no node, no build step | A toolchain adds a second language ecosystem, a `node_modules` tree, and a compile step to a repository whose only runtime dependency was `typer`, and none of that shows up on screen — a commit-graph renderer is the one piece a chart library wouldn't have given us anyway, and lane assignment against a real DAG is ~110 lines. Reversible in the direction that matters: the REST layer is a plain JSON API, so a React front end could be dropped in later against the same endpoints without touching Python. |
| REST surface | Read-only, plus `POST /api/replay`. No `remember`/`forget`/`commit`/`checkout`/branch-create over HTTP | Auth is slice 11's row, so anything writable shipped here would be an unauthenticated write path into memory for a whole slice — the same gap the MCP server already discloses as a caveat, and repeating a known gap deliberately is different from repeating it twice. There is also nothing to gain: MCP already owns the write path, and it owns it with a staging model (`refs/memgit/staging/<key>`) an HTTP POST would have to duplicate or bypass. Replay is the one exception because it *cannot* write — `ablate_and_replay` never calls `Repository.commit` by design — so it is a read of the model, not of memory. |
| Replay's LLM client | A FastAPI dependency (`deps.get_llm_client`) whose default is `default_client()`, overridden in tests with `conftest.ScriptedClient` via `app.dependency_overrides` | `LLMClient` was made a one-method Protocol precisely so a fake is one object; `dependency_overrides` is the ASGI-side equivalent of the `_agent_client` monkeypatch seam `cli.py` already uses, so the endpoint is tested end-to-end — routing, validation, response model, `changed` — with zero network and zero API key. Constructing the client in the dependency rather than the route is also what turns "the `agent` extra isn't installed" into a 503 with an install hint instead of a 500 with a traceback. |
| Pydantic vs. `to_dict` | Models own every request and every response the API composes (`Head`, commits, retrieval, ablation — the types with no `to_dict`, plus `Commit`, whose `to_dict` deliberately omits its own hash). `Diff` and `MemoryState` pass through as the dicts core already emits, inside a thin typed envelope | This row is titled *API validation*, and validation belongs at the trust boundary: untrusted input, and shapes nothing else defines. `Diff.to_dict()` is already this project's diff contract — the eval suite reads it, `--json` consumers read it, and it embeds the cardinality map it used so a result stays auditable. A Pydantic mirror of it would be a second definition of one schema (eight `ChangeKind`s, six `ValueKind`s, `KeyDiff`, `ValueChange`, `DiffStat`, nested `Fact`), untested against the payload `tests/test_diff.py` actually asserts on, and it would re-validate trusted internal data on every response — so the first drift between the two surfaces as a 500 on a working diff. A key-set guard in `tests/test_api_models.py` buys what the mirror would have, at a tenth the surface. |
| Commit payloads over the wire | Every commit the API returns carries its `hash`, attached by the API layer; `/api/log` walks with `core.graph.walk`, not `Repository.log` | `Commit.to_dict()` is the *object* payload — the thing that gets hashed — so the hash cannot be inside it without being self-referential, and `Repository.log` yields `Commit` objects that have already dropped the hash `walk` computed. A wire commit is a view, not an object, and the view needs an identity: without it the graph's parent references point at nodes the response never named, and it fails silently rather than loudly. |
| Exception mapping | Centralized `add_exception_handler` per exception type (404 / 503 / 500 / 502), with exactly one per-route `except ValueError` in `/api/diff` | The status a `RevisionNotFoundError` deserves is a property of the exception, not of the route, so putting it in eight routes makes it eight facts that can disagree — and the handler also catches what a route-level `try` cannot, namely failures inside dependencies. `cli.py` stays per-command because there the *message* is the UX and it differs per command; over HTTP there is one envelope. The merge-base `ValueError` stays local on purpose: a global `ValueError` handler would convert every genuine bug in the API layer into a polite 400, which is exactly how a broken service looks healthy. |
| Repository lifetime | One instance, discovered from cwd at `serve-web` startup and injected by a dependency; `serve-web` exits before binding if there is no repository | Freshness costs nothing to give up, because there is nothing cached to go stale: `head()`, `branches()`, `config()`, `cardinality()` and `decay()` all re-read from disk per call, so a single instance still sees commits made by a `memgit ask` in another terminal. Per-request `discover()` would re-walk the filesystem for the same answer and would silently follow a changed process cwd. Injecting it is also the seam that lets every test build an app over a `tmp_path` repo without `chdir`, matching `mcp/server.py`'s `build_server(repo)` split. |
| Vector-cache writes under concurrency | One process-wide `threading.Lock` (`deps.retrieval_lock`) around any API retrieval call that can write vectors | `VectorIndex` is single-writer by construction — append to the pack, then read-modify-write `index.json` — and until now every caller was serial (one CLI process, one stdio MCP loop). FastAPI runs sync routes in a threadpool, so the HTTP layer is the project's first genuinely concurrent caller, and two cold `recall`s racing lose offset records to a last-writer-wins rewrite. Serializing retrieval costs nothing for a single-user debugging dashboard, and a warm cache (`memgit embed`) makes the locked section a pure read. |
...
- **React, or any build step, for the dashboard** (slice 10) — see the Tech Stack table's "Dashboard" row; the REST layer stays plain JSON either way, so nothing here is a one-way door.
- **Write endpoints over HTTP** (slice 10) — `remember`/`forget`/`commit`/`checkout`/branch-create all stay MCP- and CLI-only. Auth doesn't exist until slice 11, and MCP already owns the write path with a staging model this API would otherwise have to duplicate or bypass.
- **Graph zoom, pan, or search; a live-refresh push channel (SSE/websocket)** (slice 10) — a manual reload is an honest interaction model for a single-user debugging tool, and a push channel would be the first thing in the project needing a real concurrency story beyond the one retrieval lock this slice already added.
